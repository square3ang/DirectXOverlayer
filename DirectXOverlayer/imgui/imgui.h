...
namespace ImGui
{
    ...
    // Widgets: Text
    IMGUI_API void          TextUnformatted(const char* text, const char* text_end = NULL, std::vector<std::pair<std::pair<int, int>, ImVec4>>* cols = NULL); // raw text without formatting. Roughly equivalent to Text("%s", text) but: A) doesn't require null terminated string if 'text_end' is specified, B) it's faster, no memory copy is done, no buffer size limits, recommended for long chunks of text.
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddText(const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end = NULL, float shadow_size = 0.0f, ImU32 shadow_start = IM_COL32_BLACK_TRANS, ImU32 shadow_end = IM_COL32_BLACK_TRANS, std::vector<std::pair<std::pair<int, int>, ImVec4>>* cols = NULL);
    IMGUI_API void  AddText(const ImFont* font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end = NULL, float wrap_width = 0.0f, const ImVec4* cpu_fine_clip_rect = NULL, float shadow_size = 0.0f, ImU32 shadow_start = IM_COL32_BLACK_TRANS, ImU32 shadow_end = IM_COL32_BLACK_TRANS, std::vector<std::pair<std::pair<int, int>, ImVec4>>* cols = NULL);
