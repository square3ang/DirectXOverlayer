...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags, std::vector<std::pair<std::pair<int, int>, ImVec4>>* cols)
{
        ...
        if (line < text_end)
        {
            ...
            while (line < text_end)
            {
                ...
                if (!line_end)
                    line_end = text_end;
                text_size.x = ImMax(text_size.x, CalcTextSize(line, line_end).x);
                RenderText(pos, line, line_end, false, cols);
                line = line_end + 1;
    ...
    else
    {
        ...
        RenderTextWrapped(bb.Min, text_begin, text_end, wrap_width, cols);
    }
}

void ImGui::TextUnformatted(const char* text, const char* text_end, std::vector<std::pair<std::pair<int, int>, ImVec4>>* cols)
{
    TextEx(text, text_end, ImGuiTextFlags_NoWidthForLargeClippedText, cols);
}
