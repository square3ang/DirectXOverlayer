...
{
	//logger.Log.Log("OnPresent: %p", &OnPresent);
	//logger.Log.Log("OnResizeBuffers: %p", &OnResizeBuffers);
...
IDXGISwapChain* DirectXHook::CreateDummySwapChain()
{
	...
	if (FAILED(result))
	{
		...
		//logger.Log.Log("D3D11CreateDeviceAndSwapChain failed: %s", error.ErrorMessage());
		return nullptr;
	}

	//logger.Log.Log("D3D11CreateDeviceAndSwapChain succeeded");
...
ID3D12CommandQueue* DirectXHook::CreateDummyCommandQueue()
{
	...
	//logger.Log.Log("Command queue: %p", dummyCommandQueue);
	...
	uintptr_t* resizeBuffersReturnAddress)
{
	...
	//logger.Log.Log("SwapChain VMT base address: %p", vmtBaseAddress);
	//logger.Log.Log("SwapChain VMT Present index: %p", vmtPresentIndex);
	//logger.Log.Log("SwapChain VMT ResizeBuffers index: %p", vmtResizeBuffersIndex);
	...
	//logger.Log.Log("Present address: %p", presentAddress);
	//logger.Log.Log("ResizeBuffers address: %p", resizeBuffersAddress);
	...
	uintptr_t* executeCommandListsReturnAddress)
{
	...
	//logger.Log.Log("CommandQueue VMT base address: %p", vmtBaseAddress);
	//logger.Log.Log("ExecuteCommandLists index: %p", vmtExecuteCommandListsIndex);
	...
	//logger.Log.Log("ExecuteCommandLists address: %p", executeCommandListsAddress);

	bool hookIsPresent = MemoryUtils::IsAddressHooked(executeCommandListsAddress);
	if (hookIsPresent)
	{
		//logger.Log.Log("Hook already present in ExecuteCommandLists");
	}
